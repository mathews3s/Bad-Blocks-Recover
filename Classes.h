...
{
private:
    HANDLE Disk ;                                                   // Your HDD or SSD;
    ULONGLONG Disk_Size ;                                           // Size of the drive, in bytes;
...
public:
    ...
    int Create_Me()                                                 // Function to create disk-file;
    {
        try                                    
        {
            ...
            if (Disk == INVALID_HANDLE_VALUE) throw My_Exceptions("ERROR! Drive wasn't found!");
        }
    ...
    int Get_My_Info()
    {
        ...
        Disk_Size = Disk_Geometry.DiskSize.QuadPart;
        Bytes_Per_Sector_Count = Disk_Geometry.Geometry.BytesPerSector;
        Cylinders_Count = Disk_Geometry.Geometry.Cylinders.QuadPart;
        Tracks_Per_Cylinder_Count = Disk_Geometry.Geometry.TracksPerCylinder;
        Sectors_Per_Track_Count = Disk_Geometry.Geometry.SectorsPerTrack;
        Sectors_Count = Disk_Geometry.Geometry.Cylinders.QuadPart * Disk_Geometry.Geometry.TracksPerCylinder
            * Disk_Geometry.Geometry.SectorsPerTrack;

        cout << "                               DRIVE INFO                                 " << endl;
        cout << " ________________________________________________________________________ "<< endl;
        cout << "|                                                                         "<< endl;
        cout << "| Partition's size in bytes: " << Partition_Info.PartitionLength.QuadPart  << endl;
        cout << "| Disk's size in bytes:      " << Disk_Size                                << endl;
    ...
    int Remap_Me(HANDLE DRIVE, BYTE* BUFFER, DWORD BUFFER_SIZE, DWORD dwBytes, LONG Current_Position)
    {
        LONG Repeat = 10000;                                       // Number of iterations while trying call remap;
        BOOL Read_Result = NULL;                                   // Result of reading;
        DWORD Position_Now;                                        // Current position in file;
        while (Repeat != NULL)
        {
            Position_Now = SetFilePointer(DRIVE, 0, NULL, FILE_CURRENT);                // Check current position;
            try
            {
                Read_Result = ReadFile(DRIVE, BUFFER, BUFFER_SIZE, &dwBytes, NULL);     // Read one sector;
            ...
            SetFilePointer(DRIVE, Current_Position, NULL, FILE_BEGIN);              // Set file position at back to read current sector again;
            Repeat--;
        }
        return 1;
    }
    int Diagnostic_Me()
    {
        DWORD Current_Position = NULL;                              // Current position in file;
        ULONGLONG Distance_To_Move = NULL;                          // Distance witch we should pass;
        DWORD Buffer_Size = NULL;                                   // Size of buffer for reading;
        BYTE* Buffer = NULL;                                        // Buffer for reading;
        DOUBLE Seconds = NULL;                                      // Current time;
        DOUBLE Time_Out = NULL;                                     // How much seconds are passed;
        BOOL Read_Result = NULL;                                    // Result of reading;  
        DWORD Bytes_Readed = NULL;                                  // Bytes returned after reading;
        ULONGLONG Checked_Sectors = NULL;                           // Count of checked sectors;
        BOOL Remap_Result = NULL;                                   // Result of remap operation
        ULONGLONG Size = Disk_Size;                                 // Size of disk to monitor reading operation
        int Input = NULL;                                           
        ...
        do
        {
            Current_Position = SetFilePointer(Disk, 0, NULL, FILE_CURRENT);                 // check current position in file                                                            
            Seconds = App_Seconds();                                                        // start check time
            try
            {
                Read_Result = ReadFile(Disk, Buffer, Buffer_Size, &Bytes_Readed, NULL);     // Read one sector;
                if (!Read_Result) throw My_Exceptions("ERROR! Problems while reading disk!");
            }
            catch (My_Exceptions& tmp)
            {
            ...
            SetFilePointer(Disk, Distance_To_Move, NULL, FILE_BEGIN);                       // move pointer back at size of readed bytes
            Current_Position = SetFilePointer(Disk, 0, NULL, FILE_CURRENT);                 // check current position in file
            ...
            case 500:
                ...
                try
                {
                    Remap_Result = Remap_Me(Disk, Buffer, Buffer_Size, Bytes_Readed, Current_Position);     // Read one sector;
            ...
            SetFilePointer(Disk, Distance_To_Move, NULL, FILE_BEGIN);            // move pointer forward at size of readed bytes 
            Checked_Sectors++;                                                   // inc count of checked sectors
        } while (Read_Result || Size >= NULL);

        CloseHandle(Disk);
        Put_Media(0, 23, (unsigned char*)" SUCCESFULL!", 14, GREEN);
        cout << "About " << Checked_Sectors << " from "<< Sectors_Count << " was checked for response" << endl;
        return 1;
    }
};
